...
class AtomicDatasetReader(DatasetReader):
    ...
    def __init__(self,
        ...
        print("Build reader successfully")
        super().__init__(lazy)
    ...
    def _read(self, file_path):
        with open(cached_path(file_path), "r") as data_file:
            ...
            for (line_num, line_parts) in enumerate(reader):
                ...
                oreacts = json.loads(line_parts[2])

                if not self._dummy_instances_for_vocab_generation:
                    for oreact in oreacts:
                        if oreact != "none":
                            yield self.text_to_instance(
                                source_sequence, oreact
                            )

                else:
                    for oreact in oreacts:
                        if oreact != "none":
                            yield self.text_to_instance(source_sequence, oreact)
