...
@ Model.register("atomic")
class Atomic(Model):
    """
    This ``Atomic`` class is a :class:`Model` which takes an event
    ...
    def __init__(self,
        ...
        target_names = target_names or ["oreact"]
    ...
    # 用bleu来做评估
    def _update_bleu(self,
                     all_top_k_prediction: torch.LongTensor,
                     target_tokens: Dict[str, torch.LongTensor],
                     target_bleu: BLEU) -> None:
        targets = target_tokens["tokens"]
        print("target:", targets)
        target_bleu(
            all_top_k_prediction,
            targets
        )
    ...
    def forward(self,  # type: ignore
        ...
        if not self.training:
            ...
            for name, state in self._states.items():
                ...
                # 这一部分在做evaluation,要改动的话，只需要改动_update_recall()函数即可
                if target_tokens:
                    self._update_recall(all_top_k_predictions, target_tokens[name], state.recall)
                # 用bleu做评估：从top_k_prediction中取出一个/多个，扔到_update_bleu中去
                # if target_tokens:
                #     self._update_bleu(all_top_k_predictions[:, 0, :], target_tokens[name], state.bleu)
                #     self._update_bleu(all_top_k_predictions[:, 1, :], target_tokens[name], state.bleu)
                #     self._update_bleu(all_top_k_predictions[:, 2, :], target_tokens[name], state.bleu)
                #     # print("prediction", all_top_k_predictions[:, 0, :])
                #     # print("target:", target_tokens[name])
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        # if not self.training:
        #     for name, state in self._states.items():
        #         # all_metrics[name] = state.bleu.get_metric(reset=reset)
        #         # all_metrics.update(self._bleu.get_metric(reset=reset))
        #         all_metrics.update(state.bleu.get_metric(reset=reset))
        return all_metrics
